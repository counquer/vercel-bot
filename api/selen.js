...
module.exports = async (req, res) => {
  ...
  if (req.method !== "POST") {
    return res.status(405).json({ error: "M谷todo no permitido, usa POST" });
  }

  const authHeader = req.headers["x-vercel-protection-bypass"];
  if (authHeader && authHeader !== VERCEL_AUTOMATION_BYPASS_SECRET) {
    return res.status(401).json({ error: "Acceso no autorizado: protecci車n Vercel activa." });
  }
  ...
  try {
    ...
    if (cache.has(cacheKey)) {
      ...
      if (Date.now() - timestamp < CACHE_TTL) {
        console.log("Respondiendo desde cach谷");
    ...
    const promptFinal = `Selen, responde con toda tu simbiosis y contexto hist車rico:\n\n${contenidos.join("\n---\n")}`;
    ...
    await fetch(NOTION_PAGE_URL, {
      ...
      body: JSON.stringify({
        ...
        properties: {
          Name: { title: [{ text: { content: "Resumen de memoria simbi車tica" } }] },
    ...
    console.error("Error en la funci車n /api/selen:", error.message);
